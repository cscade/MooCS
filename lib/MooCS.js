...
MooCS.Device = new Class({
	...
	initialize: function (location, startup) {
		var controller = this;
		...
		this.read('system', 'model', function (r) {
			if (r === 'BCS-460') {
				...
				Object.merge(controller.readDict, MooCS.readDict460);
				Object.merge(controller.writeDict, MooCS.writeDict460);
			} else if (r === 'BCS-462') {
				controller.BCS462 = true;
				Object.merge(controller.readDict, MooCS.readDict460, MooCS.readDict462);
				Object.merge(controller.writeDict, MooCS.writeDict460, MooCS.writeDict462);
			} else {
				// Unsupported controller
				alert('Controller could not be contacted or controller detection returned an unknown controller type.\n\nCheck your network connectivity and verify that you are using a compatible controller model.');
				return;
			}
	...
	list: function (area) {
		...
		Object.each(this[(area === 'read') ? 'readDict' : 'writeDict'], function (members, key) {
			var what = [];
	...
	read: function (section, key, callback) {
		...
		if (this.readDict[section] === undefined || (this.readDict[section].structure && this.readDict[section].structure[key] === undefined)) {
	...
	hardRead: function (section, key, callback) {
		...
		delete this.responseCache[this.readDict[section].target];
	...
	performRead: function (section, key, callback) {
		...
		var readable = this.readDict[section],
		...
		if (this.responseCache[readable.target] !== undefined && (readable.cacheable || this.responseCache[readable.target].asOf > (Date.now() - 500))) {
			decoded = this.decode(this.responseCache[readable.target].value, decodeOpts);
			...
			r = new Request({
				...
				onSuccess: function (responseText) {
					...
					controller.responseCache[readable.target] = { asOf: Date.now(), value: responseText };
					callback.apply(callback, [decoded]);
					controller.nextQueue();
				}
			}).send();
