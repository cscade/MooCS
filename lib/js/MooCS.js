...
MooCS.Device = new Class({
	...
	read: function (section, key, callback) {
		...
		if (this.readDict[section] === undefined || this.readDict[section][key] === undefined) {
	...
	performRead: function (section, key, callback) {
		...
		if (this.responseCache[decodeOpts.target] !== undefined && (MooCS.cacheable[decodeOpts.target] || this.responseCache[decodeOpts.target].asOf > (Date.now() - 500))) {
